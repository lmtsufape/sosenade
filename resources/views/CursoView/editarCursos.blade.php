...
@section('content')
    ...
    <div class="shadow p-3 bg-white" style="border-radius: 10px; width: 80%; margin-left: auto; margin-right: auto">
        ...
        <form action="{{route('update_curso')}}" method="post">
            ...
            <div class="form-group justify-content-center row">
                <div class="form-group col-md-4">
                    ...
                    <input type="text" class="form-control{{ $errors->has('curso_nome') ? ' is-invalid' : '' }}"
                           ...
                           value="{{ $curso->curso_nome }}" required autofocus>
                ...
                <div class="form-group col-md-4">
                    ...
                    <select class="form-control{{ $errors->has('ciclo_id') ? ' is-invalid' : '' }}" name="ciclo_id"
                        ...
                        @foreach ($ciclos as $ciclo)
                            <option
                                value="{{$ciclo->id}}" {{ (old('ciclo') == $ciclo->id) || ($curso->ciclo_id == $ciclo->id) ? 'selected' : ''}} >{{$ciclo->tipo_ciclo}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-row col-md-12 justify-content-center">
                    <div class="form-group col-md-4">
                        ...
                        <select class="form-control{{ $errors->has('unidade_id') ? ' is-invalid' : '' }}"
                                name="unidade_id"
                                required autofocus>
                            <option hidden disabled selected>Selecione a unidade</option>
                            @foreach($unidade_academicas as $unidade_academica)
                                <option
                                    value="{{$unidade_academica->id}}" {{ (old('unidade_academica') == $unidade_academica->id) || ($curso->unidade_id == $unidade_academica->id) ? 'selected' : ''}}>{{$unidade_academica->nome}}</option>
                            @endforeach
                        </select>
